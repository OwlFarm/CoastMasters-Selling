...
class ScraperIntegration:
    ...
    def export_scraper_listings(self, collection_name: str = None) -> List[Dict[str, Any]]:
        """
        Export yacht listings from Firestore scraper collection
        Tries multiple collection names automatically if none specified
        Returns structured data ready for RAG processing
        """
        try:
            # If no collection specified, try to find yacht data automatically
            if collection_name is None:
                collection_name = self._find_yacht_collection()
                if not collection_name:
                    print("❌ No yacht data collections found")
                    return []
    ...
    def _find_yacht_collection(self) -> str:
        """
        Automatically find the collection containing yacht listings
        """
        print("🔍 Searching for yacht data collections...")
        
        # Common collection names for yacht listings
        possible_names = [
            'yacht_listings', 'listings', 'yachts', 'boats', 'vessels',
            'broker_listings', 'marine_listings', 'sales', 'yacht_sales',
            'boat_listings', 'vessel_listings', 'marine_sales'
        ]
        
        all_collections = list(self.db.collections())
        print(f"📁 Found {len(all_collections)} total collections")
        
        # First, try exact matches
        for name in possible_names:
            if any(col.id == name for col in all_collections):
                print(f"🎯 Found exact match: {name}")
                return name
        
        # If no exact matches, look for collections with yacht-related keywords
        yacht_keywords = ['yacht', 'boat', 'vessel', 'marine', 'listing', 'sale', 'broker']
        
        for collection in all_collections:
            collection_name = collection.id.lower()
            if any(keyword in collection_name for keyword in yacht_keywords):
                print(f"🎯 Found potential match: {collection.id}")
                return collection.id
        
        # If still nothing, check collections with actual documents
        print("🔍 Checking collections for yacht-like data...")
        for collection in all_collections:
            try:
                docs = list(collection.stream())
                if docs > 0:
                    # Check if this looks like yacht data
                    sample_doc = docs[0].to_dict()
                    if self._looks_like_yacht_data(sample_doc):
                        print(f"🎯 Found yacht-like data in: {collection.id}")
                        return collection.id
            except Exception as e:
                continue
    ...
    def _looks_like_yacht_data(self, data: Dict[str, Any]) -> bool:
        """
        Check if document data looks like yacht/boat information
        """
        yacht_indicators = [
            'name', 'make', 'model', 'year', 'length', 'price',
            'yacht', 'boat', 'vessel', 'marine', 'sailing',
            'hull', 'keel', 'sail', 'engine', 'fuel'
        ]
        
        # Check field names
        data_keys = [key.lower() for key in data.keys()]
        yacht_field_matches = sum(1 for indicator in yacht_indicators if any(indicator in key for key in data_keys))
        
        # Check field values
        yacht_value_matches = 0
        for value in data.values():
            if isinstance(value, str):
                value_lower = value.lower()
                yacht_value_matches += sum(1 for indicator in yacht_indicators if indicator in value_lower)
        
        # If we have multiple yacht indicators, this is likely yacht data
        return yacht_field_matches >= 2 or yacht_value_matches >= 3
